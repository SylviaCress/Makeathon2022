...
const float degPerClick = 360.0 / 40.0;
const float stepsPerDeg = 2038.0 / 360;
//Section 2
...
volatile float vaneAngle = 0; //set to 0 at setup
...
void loop() {
  ...
  // Do this in the loop before we send it
    if (vaneAngle > 360) {
      vaneAngle = vaneAngle - 360.0;;
    } else if (vaneAngle < 0) {
      vaneAngle = 360.0 + vaneAngle;
    }
...
String soundData()
{
  ...
  return String(currentSound/1023.0*5.0); //converting to voltage
}
...
String moveStepper(float desiredAngle)
{
  ...
  if(abs(vaneAngleToMove) > 180) {
      if(vaneAngleToMove < 0){
         vaneAngleToMove += 360;
      } else {
         vaneAngleToMove -= 360;
      }
